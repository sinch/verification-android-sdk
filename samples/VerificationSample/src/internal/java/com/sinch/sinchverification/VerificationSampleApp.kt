...
class VerificationSampleApp : Application() {
    ...
    fun onDevelopmentOptionSelected(item: MenuItem): Boolean {
        ...
        globalConfig = when (item.itemId) {
            ...
            R.id.productionAPSE1 -> buildGlobalConfig(
                BuildConfig.API_BASE_URL_PROD_APSE1,
                BuildConfig.APP_KEY_PROD_APSE
            )
            R.id.productionEUC1 -> buildGlobalConfig(
                BuildConfig.API_BASE_URL_PROD_EUC1,
                BuildConfig.APP_KEY_PROD_EUC1
            )
